...
## 📲 Stay logged in / installing on a phone

- **Refresh tokens last 180 days and rotate on every use** (`SIMPLE_JWT` in `backend/config/settings.py`) — the frontend's axios interceptor (`frontend/src/services/api.js`) swaps an expired access token for a new one automatically, so a session survives as long as the app is opened at least once every 180 days. No separate "stay logged in" checkbox is needed.
- **Push notifications** are delivered by the browser's push service, not by the open tab — they arrive even when the app is closed or the JWT session has logged out, as long as the subscription (registered once via the Settings page) is still valid.
- **Home Screen on iPhone**: Safari → Share → "Add to Home Screen" (`frontend/public/manifest.webmanifest` + `apple-mobile-web-app-capable` make it open full-screen, no browser chrome). iOS only allows Web Push for a page added to the Home Screen — open the app from the Home Screen icon at least once, log in, then enable push in Settings from there.
...
- **Shopping Lists**: multiple lists, each with a favorite flag (the Cooking Plan will write into it) and optional per-member visibility; items carry a quantity and unit, autocomplete from the ingredient catalogue, are editable in place (title/quantity/unit) after creation, and completed items can be cleared in one click
- **Shopping history**: ticking an item off logs a purchase (a snapshot, so it survives deleting the item or clearing completed ones; un-ticking undoes it). The Shopping page's "History" tab shows the selected list's most-bought items (with "add again", which adds to that same list) and recent purchases by day, searchable and filterable by period
...
- **Cooking Plan**: a weekly meal plan (lunch/dinner by default, configurable) -- edited on its own page, `/cooking-plan`. Dishes are picked from five suggestion buckets per meal type — *craving* (rating and "not cooked lately" combined), *best rated*, *not cooked in a while*, *random picks*, *rest* — or by search/label; a **free dish** (no recipe -- ready meal, takeaway...) needs just a name; a dish can be re-planned as "leftovers of ..." (no task, no extra shopping) but only into a slot *after* the source dish (a later day, or a later meal the same day) -- enforced on create/edit and when dragging the cook task, and a snoozed dish carries its leftovers along by the same number of days. Cooking plan config's `default_excluded_from_shopping_list` per ingredient controls which staples start unticked in the shopping step. Planning is **step 1 of the weekly household planning** (a modal offers it first, with a status line and a skip) and is also driven by its own recurring **weekly meal planning** task ("Plan now" opens the plan for next week). "Finish plan" creates **one cook task per dish/free dish** in the household plan, where it is assigned and dragged like any task (moving/skipping/snoozing/deleting the task moves/removes the dish), then previews every remaining dish's shopping lines (untick dishes/ingredients, merges equal ingredient+unit) before returning to household planning. Ticking a cook task off logs the cooking (`MealEvent`, skipped for free dishes) and asks for a rating if the user has none yet and the dish has a recipe
- **Dashboard**: welcome message, a KPI overview (shopping/recipes/meals/overdue tasks) with shortcuts, "my open household tasks" grouped by overdue/today/this-week followed by the rest of the household's overdue tasks in the same card, this week's completion progress (overall plus per member), today's planned meals with a one-click "mark cooked", and a read-only weekly preview
...
- **Notifications**: email (SMTP) and Web Push, per-type/per-channel toggles, for tasks due today, weekly household planning due, weekly meal planning due and "Cooking today: <dish>" (goes to the assignee, or to the whole household while unassigned, from 08:00) — **each member picks their own notification language in Settings** (German by default, independent of the browser's app language), so a household can read email/push in two languages; delivered by a cron-driven management command, not a background daemon; test-email/test-push buttons in Settings for on-demand verification
...
- `GET /api/purchases/` (`?list=`, `?q=`, `?start=&end=`), `GET /api/purchases/summary/` (most-bought, same filters, `?limit=`) - Read-only purchase history
...
- `POST /api/shopping-lists/{id}/add-ingredients/` - Add (merge) ticked ingredient lines; optional `entries: [id, ...]` stamps those cooking-plan dishes as `shopping_added_at` (so a later shopping-preview shows "already on the list" instead of offering to re-add automatically); `GET /api/recipes/{id}/shopping-lines/?servings=` - one recipe's scaled lines
...
- `GET/PATCH /api/notification-preferences/` - Current user's per-type email/push toggles (rows auto-created on first access); notification language lives on `HouseholdMember.notification_language`, editable via `PATCH /api/members/{id}/`
...
- **CookingPlanEntry**: one planned meal — date, meal type, kind (cook / leftovers / free), recipe (null for `free`, which has a plain `title` instead), servings; a cook/free entry owns one `HouseholdTaskInstance` (`system_action=cook_meal`) and, once cooked (recipe-backed only), a `MealEvent`
